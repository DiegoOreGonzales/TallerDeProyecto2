...
| **RS-06** | Técnico | Fugas de conexiones en la base de datos relacional PostgreSQL debido a sesiones abiertas de forma indefinida por llamadas concurrentes de la API. | 3 | 4 | **12 (Medio)** | Se reestructuró la creación de sesiones en `database.py` utilizando generadores contextuables `yield` en FastAPI, garantizando que cada petición cierre su respectiva conexión al terminar. | 1 | 2 | **2 (Bajo)** | **Mitigación Continua** | **Mitigado & Cerrado** |
| **RS-07** | Usabilidad / Operativo | Baja adopción o rechazo del sistema por parte de coordinadores académicos y docentes debido a la complejidad de la interfaz de horarios. | 3 | 3 | **9 (Medio)** | Realización de un estudio métrico cuantitativo de usabilidad mediante el test estandarizado SUS, aplicando mejoras inmediatas basadas en la retroalimentación de 10 usuarios simulados. | 1 | 2 | **2 (Bajo)** | **Aceptación Activa** | **Mitigado & Cerrado** |

---

## 🔑 3. Lecciones del Control de Riesgos

La gestión activa de riesgos a través del Product Owner y Scrum Master permitió evitar la materialización de incidentes críticos de pérdida de datos o fallos de compilación en producción. La integración de la seguridad y calidad como parte del Backlog desde el inicio del proyecto redujo el impacto de los riesgos técnicos en un 80%, garantizando un sistema robusto, seguro y altamente utilizable.
